...
const BOOT_LINES = [
  { text: '[    0.000000] Linux version 7.0.9-204.fc44.x86_64 (gcc 14.1.1) (Fedora Linux)',  cls: '' },
  { text: '[    0.000001] Command line: BOOT_IMAGE=/vmlinuz root=/dev/mapper/fedora-root ro',  cls: '' },
  { text: '[    0.012834] BIOS-provided physical RAM map: 32768 MB',  cls: '' },
  { text: '[    0.023415] ACPI: BIOS IRQ0 pin2 override',  cls: '' },
  { text: '[    0.189234] PCI: Using configuration type 1 for base access',  cls: '' },
  { text: '[    0.291847] systemd[1]: Starting jeet.jawale.portfolio.service...',  cls: 'boot-info' },
  { text: '[    0.399201] USB: keyboard detected — Cherry MX Blues. Nice.',  cls: '' },
  { text: '[    0.445123] NVIDIA RTX: ready to cook',  cls: 'boot-ok' },
  { text: '[    0.512334] Loading neural network weights from checkpoint...',  cls: '' },
  { text: '[    0.567890] checkpoint OK  (loss: 0.0012, acc: 99.8%)',  cls: 'boot-ok' },
  { text: '[    0.623456] Mounting /home/jeet/projects — 4,891 commits attached',  cls: '' },
  { text: '[    0.734567] SSH agent: identity loaded — jeet_ed25519',  cls: 'boot-ok' },
  { text: '[    0.801234] Syncing node_01... [████████████] 100%',  cls: 'boot-ok' },
  { text: '[    0.867891] Syncing node_02... [████████████] 100%',  cls: 'boot-ok' },
  { text: '[    0.923456] Syncing node_03... [█████████░░░]  75% — almost there',  cls: 'boot-warn' },
  { text: '[    0.987654] Syncing node_03... [████████████] 100%',  cls: 'boot-ok' },
  { text: '[    1.034567] Distributed compute: all nodes synced',  cls: 'boot-ok' },
  { text: '[    1.145678] bash: work-life-balance: command not found',  cls: 'boot-err' },
  { text: '[    1.256789] Portfolio service: fully operational',  cls: 'boot-ok' },
  { text: '',  cls: '' },
  { text: '[  OK  ] Reached target: jeet.jawale.portfolio — Welcome █', cls: 'boot-ok' },
];
...
function startBootSequence() {
  ...
  return new Promise(resolve => {
    ...
    function showNext() {
      ...
      appendLine(BOOT_LINES[i]);
      const delay = BOOT_LINES[i].text === '' ? 80
        : BOOT_LINES[i].cls === 'boot-ok'  ? Math.random() * 40 + 25
        : BOOT_LINES[i].cls === 'boot-err' ? 180
        : Math.random() * 60 + 30;
      i++;
...
function initNeuralCanvas() {
  ...
  function triggerPulse() {
    ...
    for (const n of nodes) {
      const dx = n.x - source.x, dy = n.y - source.y;
      const dist = Math.sqrt(dx*dx + dy*dy);
      if (dist < CFG.connDist && n !== source) {
        setTimeout(() => { n.act = Math.max(n.act, 0.75 * (1 - dist / CFG.connDist)); },
          50 + dist * 0.6);
      }
    }
  }
...
Tip: use ↑↓ arrow keys for command history.`,
    ...
    'ls projects/': () =>
`total 3 (use 'cat <project>' for details)

drwxr-xr-x  heritage-explorer/        Aug 2024 → Jun 2025  [DEPLOYED ✓]
drwxr-xr-x  course-exit-survey/       Feb 2024 → Jun 2024  [ARCHIVED]
drwxr-xr-x  c-learning-app/           Oct 2023 → Jan 2024  [STABLE]
-rw-r--r--  more-in-progress.txt      ...`,

    'cat resume.txt': () =>
`NAME:      Jeet Jawale
DEGREE:    B.Tech. Computer Engineering (2025)
LOCATION:  Navi Mumbai, India
FOCUS:     Linux · Python · Bash · DevOps · Automation

EXPERIENCE:
  BARC, Trombay (Jun–Aug 2025)
  Project Trainee — Radiation Safety Systems Division
  → PyQGIS spatial analysis, decision support workflows

PROJECTS:
  Heritage Map & Recommendation Engine  — React/MapLibre/Firebase
...
jeet      4891  99.9  0.0  thinking_about_side_projects`,
    ...
    'rm -rf /': () => {
      ...
      document.body.innerHTML = '<div style="color:#0f0; background:#000; font-family:monospace; padding:2rem; font-size:24px; width:100vw; height:100vh;">KERNEL PANIC - NOT SYNCING: Fatal exception in interrupt<br>System halted.<br>Rebooting in 3 seconds...</div>';
  ...
  input.addEventListener('keydown', e => {
    ...
    if (e.key === 'Enter') {
      ...
      if (terminalState === 'sudo') {
         ...
         addBlock('', `jeet is not in the sudoers file. This incident will be reported.`, 'cli-err');
         return;
      }
      runCommand(val);
...
function initMobileMenu() {
  ...
  toggle.addEventListener('click', () => {
    ...
    menu.setAttribute('aria-hidden', String(!menu.classList.contains('active')));
...
function initObservers() {
  ...
  observe('.projects-list', 0.04, el => {
    el.querySelectorAll('.proj-training-card').forEach((card, i) => {
      setTimeout(() => {
        ...
        if (barFill) {
          setTimeout(() => { barFill.style.setProperty('--fill-pct', pct + '%'); barFill.style.width = pct + '%'; }, 200);
        }
      }, i * 130);
    });
  });
  ...
  observe('.skills-pipeline', 0.05, el => {
    el.querySelectorAll('.pipeline-section').forEach((s, i) => setTimeout(() => s.classList.add('in'), i * 90));
  });
...
function initContactForm() {
  ...
  function isValidEmail(e)   { return emailRegex.test(e) && e.length <= 254 && !e.includes('..'); }
  function isValidName(n)    { return n.length > 2 && n.length < 100 && /^[a-zA-Z\s'-]+$/.test(n); }
  function isValidMessage(m) { return m.length > 5 && m.length < 5000; }
  ...
  form.addEventListener('submit', async function(e) {
    ...
    if (!name || !email || !message)      { setBtn('Fill all fields', 'error'); resetBtn(); return; }
    if (Date.now() - lastSubmit < 3000)   { setBtn('Please wait…', 'error');   resetBtn(); return; }
    if (!isValidName(name))               { setBtn('Invalid name', 'error');    resetBtn(); return; }
    if (!isValidEmail(email))             { setBtn('Invalid email', 'error');   resetBtn(); return; }
    if (!isValidMessage(message))         { setBtn('5–5000 characters', 'error'); resetBtn(); return; }
...
(function() {
  ...
  async function fetchContributions() {
    ...
    try {
      ...
      if (data.totalContributions !== undefined) {
        ...
        total = data.contributions.flat().reduce((s, d) => s + (d.contributionCount || 0), 0);
      }
  ...
  async function fetchPRs(state) {
    ...
    try {
      ...
      prList.innerHTML = '<div class="pr-loading">Error loading data. Check connection.</div>';
    }
  }
...
function initFloatingTerminal() {
  ...
  let preMaxState = { width: '', height: '', top: '', left: '', bottom: '', right: '' };
  ...
  btnMax.addEventListener('click', () => {
    ...
    if (terminal.classList.contains('term-maximized')) {
      ...
      preMaxState = {
        ...
        top: wrapper.style.top, left: wrapper.style.left,
        bottom: wrapper.style.bottom, right: wrapper.style.right
      };
  ...
  function onResizeDrag(e) {
    ...
    if (newWidth < minWidth) {
        newWidth = minWidth;
        if (currentResizer.classList.contains('resizer-sw') || currentResizer.classList.contains('resizer-nw')) {
            newLeft = rStartLeft + (rStartWidth - minWidth);
        }
    }
    if (newHeight < minHeight) {
        newHeight = minHeight;
        if (currentResizer.classList.contains('resizer-nw') || currentResizer.classList.contains('resizer-ne')) {
    ...
    if (newLeft < 0) {
      if (currentResizer.classList.contains('resizer-nw') || currentResizer.classList.contains('resizer-sw')) {
        newWidth -= (0 - newLeft);
      }
      newLeft = 0;
    }
